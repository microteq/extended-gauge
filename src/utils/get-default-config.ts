...
export function getDefaultConfig(_hass: HomeAssistant): object {
  ...
  const normalColor = hexToRgb(
    rootStyles.getPropertyValue("--secondary-text-color")
  );
  const backgroundColor = hexToRgb(
    rootStyles.getPropertyValue("--primary-background-color")
  );
  const config: ExtendedGaugeConfigData = {
