...
export class ConfigFramework extends LitElement implements LovelaceCardEditor {
  ...
  public async connectedCallback() {
    ...
    try {
      ...
      this._eventSubscription = await this.hass.connection.subscribeEvents(
        async (event: any) => {
      ...
      console.error(
        "Error while subscribing to the lovelace_updated event:",
        err
      );
    }
  }
  ...
  protected valueChanged(
    ...
    _configData: any,
    _sectionName: string,
    _sectionConfigData: any,
    _previousSectionConfigData: any
  ): any {}
  ...
  protected setCurrentPage(
    ...
    isAdd: boolean | undefined = undefined
  ): void {
    ...
    if (currentPage._rowIndex != undefined)
      newPage._configData =
        currentPage._configData[currentPage._rowIndex!][selectedSection.name];
    else newPage._configData = currentPage._configData[selectedSection.name];
  ...
  protected updateFormValues(
    ...
    const sectionConfigData = ev.detail.value;
    let pageConfigData;
    let newConfigData;
    const currentPage = this._getCurrentPage();
    if (currentPage._rowIndex != undefined) {
    ...
    if (!pageConfigData[section.name]) pageConfigData[section.name] = {};
    ...
    if (isValid)
      pageConfigData = this.valueChanged(
        pageName,
        pageConfigData,
        section.name,
        sectionConfigData,
        this._previousSectionConfigData
      );
    if (currentPage._rowIndex != undefined)
      newConfigData[currentPage._rowIndex] = pageConfigData;
    else newConfigData = pageConfigData;
  ...
  private _updateSectionConfigData(
    ...
    if (sectionName == undefined) deepestLevel -= 1;
    for (let i = deepestLevel; i >= 0; i--) {
      let parentClone;
      let parentCloneArrayElement;
      const parentPage = this._siteHistory[i];
      const parentData = parentPage._configData;
      const newSectionName =
        i == this._siteHistory.length - 1
          ? sectionName!
          : parentPage._sectionName;
  ...
  private _elementListChanged(ev: CustomEvent, section: PageSection): void {
    ...
    switch (ev.detail.change) {
      case "add":
        elementList[index] = this.listElementAdded(
          section.name,
          elementList[index]
        );
      ...
      case "edit":
        ...
        break;
      case "remove": {
        const removeOk: boolean = this.listElementRemoving(
          section.name,
          elementList[index]
        );
        if (removeOk) {
    ...
    return page != undefined &&
      ...
      ? html`
          <microteq-page-header
            ...
            pageTitle=${this.localizeText(
              `${
                page._isAdd == undefined || !page._isAdd
                  ? page.title
      ...
      <div style="display: flex;">
        <ha-form
          ...
          @value-changed=${(ev: CustomEvent) =>
            this.updateFormValues(pageName, section, ev)}
        >
        </ha-form>
        <ha-icon-button
  ...
  private _renderElementList(section: PageSection): any {
    ...
    return html`
      <microteq-element-list
        ...
        .listEmptyText=${this.localizeText(
          `${section.alternative_title ?? ""}`
        )}
        .addNewText=${this.localizeText(`${section.link!.alternative_title}`)}
        @microteq-element-list-changed=${(ev: CustomEvent) =>
          this._elementListChanged(ev, section)}
      >
      </microteq-element-list>
    `;
  }
  ...
  private _renderSection(section: PageSection): any {
    let pageName;
    if (this._siteHistory.length == 1) pageName = "";
    else
      pageName =
        this._siteHistory[this._siteHistory.length - 2]._sectionName ?? "";
    switch (section.type) {
      case "menu":
