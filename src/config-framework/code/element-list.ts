...
export class ElementList extends LitElement {
  ...
  private _elementListChanged(
    ...
    fireEvent(this, "microteq-element-list-changed", {
  ...
  private _findNewElementId(elementList: any[]): number {
    ...
    const item = sortedElementList.find(
      (item, index, arr) => index > 0 && item.id !== arr[index - 1].id + 1
    );
    if (item) return item.id - 1;
    else return sortedElementList[sortedElementList.length - 1].id + 1;
  }
  ...
  private async _addRow(): Promise<void> {
    ...
    const newId = this._findNewElementId(this.elementList);
  ...
  private _rowMoved(ev: CustomEvent): void {
    ...
    const newElementList = this.elementList!.concat();
    newElementList.splice(
      ev.detail.newIndex!,
      0,
      newElementList.splice(ev.detail.oldIndex!, 1)[0]
    );
  ...
  protected render(): TemplateResult {
    ...
    return html`
      <div class="title">
        ${this.elementList && this.elementList.length > 0
          ? this.listTitle
          : this.listEmptyText}
      </div>
      <ha-sortable
      ...
      >
        <mwc-list>
              ...
              <div class="${sortable ? "draggable" : ""} row-container row">
                ${sortable
                  ? html`
                    ...
                    `
                  : ""}
                <div class="row-text">${element?.title?.title}</div>
                <ha-icon-button
          ...
          (
            this.elementList![this.elementList.length - 1].title?.title ?? ""
          ).trim() == ""
            ? html``
            : html`
            <div class="row-container">
              ...
              <ha-icon-button
                .label=${this.hass!.localize(
                  "ui.components.entity.entity-picker.add"
                )}
                .path=${mdiPlus}
                class="add-icon"
                @click=${this._addRow}>
