...
export class ExtendedGaugeCard extends LitElement {
  ...
  @state() private _config: ExtendedGaugeConfigData = {
  ...
  private _setMinValue(value) {
    ...
    if (!value) return;
    else this._minValue = Math.min(value, this._minValue);
  }
  ...
  private _setMaxValue(value) {
    ...
    if (!value) return;
    else this._maxValue = Math.max(value + 0.000001, this._maxValue);
  }
  ...
  private _getValue(): number | undefined {
    ...
    if (
      ...
      this.hass?.states[this._config.entity?.entity]
    )
      entity = this.hass?.states[this._config.entity?.entity];
    if (!entity) return undefined;
    const stringValue = entity.state;
    if (stringValue == "unavailable") return undefined;
    switch (entity.attributes.device_class) {
      case "timestamp":
        return undefined;
      default:
        return this._config.entity?.settings?.conversion_factor == undefined
          ? parseFloat(stringValue)
          : parseFloat(stringValue) /
              this._config.entity?.settings?.conversion_factor;
    }
  }
  ...
  private _updateDemoValue = () => {
    DemoTimerManager.demoValue =
      this._minValue +
      Math.round((this._maxValue - this._minValue) * Math.random());
    this.requestUpdate();
  };
  ...
  protected render(): TemplateResult {
    ...
    let value;
    if (
      this._config.entity?.entity == null ||
      this.hass.states[this._config.entity?.entity] == undefined
    ) {
      ...
      <ha-card style="text-align: center !important;">
        ...
        <div class="card-content-container">
          <microteq-extended-gauge
            ...
            .valueLabel="${config.main?.show_entity_name
              ? config.entity?.settings && config.entity?.settings?.name
                ? config.entity?.settings?.name
